...
def open_import_window():
    core.open_import_window(
        tool.AttributeImport,
        tool.AttributeImportResults,
        tool.IfcImporter,
        tool.Project,
...
def connect_ifc_import_runner(runner: IfcImportRunner):
    runner.signaller.started.connect(
        lambda: core.ifc_import_started(runner, tool.AttributeImport, tool.IfcImporter)
    )
    runner.signaller.finished.connect(
        lambda: core.ifc_import_finished(runner, tool.AttributeImport, tool.IfcImporter)
    )
...
def update_ifc_type_combobox():
    core.update_ifctype_combobox(
        tool.AttributeImportResults, tool.AttributeImportSQL, tool.Project
    )
...
def pset_table_selection_changed():
    core.update_property_table(tool.AttributeImportResults, tool.AttributeImportSQL)
...
def value_checkstate_changed(check_box):
    core.value_checkstate_changed(
        check_box, tool.AttributeImportResults, tool.AttributeImportSQL
    )
...
def all_checkbox_checkstate_changed():
    core.all_checkbox_checkstate_changed(
        tool.AttributeImportResults, tool.AttributeImportSQL
    )
...
def settings_clicked():
    core.settings_clicked(
        tool.AttributeImportResults, tool.AttributeImportSQL, tool.Util
    )
...
def download_clicked():
    core.export_attributes(
        tool.AttributeImportResults, tool.AttributeImportSQL, tool.Appdata, tool.Popups
    )
