    ...
    def __init__(self, main_window: MainWindow):
        def create_connections():
            ...
            self.widget.table_widget_attribute.itemClicked.connect(
                lambda item: item.activate(self.widget.table_widget_value))
                ...
                for attribute, value_dict in attribute_dict.items():
                    ...
                    for value, [accept_bool, count] in value_dict.items():
                        if value not in values and accept_bool:
                            values.append(value)
                            logging.info(
                                f"Wert {value} wurde fuer {obj.name} -> {property_set.name}:{attribute.name} ergaenzt")
...
class ObjectCollection(object):
    ...
    def activate(self, property_set_table_widget: QTableWidget):
        ...
        for index, (property_set, attribute_dict) in enumerate(self.property_set_dict.items()):
            property_set_table_widget.setItem(index, 0,
                                              PropertySetItem(self, property_set, attribute_dict, property_set.name))
