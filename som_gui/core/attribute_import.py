...
def open_import_window(attribute_import: Type[tool.AttributeImport],
    ...
    if attribute_import.is_window_allready_build():
        attribute_import.get_results_window().show()
        return
...
def update_results_window(attribute_import: Type[tool.AttributeImport],
    ...
    widget = attribute_import.get_results_window().widget
    widget.combo_box_name.repaint()
    widget.combo_box_group.repaint()
    update_property_set_table(attribute_import, attribute_import_sql)
...
def open_results_window(attribute_import: Type[tool.AttributeImport]):
    attribute_import_widget = attribute_import.create_attribute_import_window()
...
def update_ifctype_combobox(attribute_import: Type[tool.AttributeImport],
                            attribute_import_sql: Type[tool.AttributeImportSQL], project: Type[tool.Project]):
    if attribute_import.is_updating_locked():
        logging.debug(f"abort Ifc combobox update because lock: {attribute_import.get_update_lock_reason()}")
        return
    ...
    update_identifier_combobox(attribute_import, attribute_import_sql, project)


def update_identifier_combobox(attribute_import: Type[tool.AttributeImport],
                               attribute_import_sql: Type[tool.AttributeImportSQL], project: Type[tool.Project]):
    if attribute_import.is_updating_locked():
        logging.debug(f"abort Identifiercombobox update because lock: {attribute_import.get_update_lock_reason()}")
        return
...
def update_property_set_table(attribute_import: Type[tool.AttributeImport],
                              attribute_import_sql: Type[tool.AttributeImportSQL]):
    if attribute_import.is_updating_locked():
        return
    ifc_type, som_object, property_set, attribute = attribute_import.get_input_variables()
    ...
    update_attribute_table(attribute_import, attribute_import_sql)


def update_attribute_table(attribute_import: Type[tool.AttributeImport],
                           attribute_import_sql: Type[tool.AttributeImportSQL]):
    if attribute_import.is_updating_locked():
        return
    ...
    update_value_table(attribute_import, attribute_import_sql)


def update_value_table(attribute_import: Type[tool.AttributeImport],
                       attribute_import_sql: Type[tool.AttributeImportSQL]):
    if attribute_import.is_updating_locked():
        return
