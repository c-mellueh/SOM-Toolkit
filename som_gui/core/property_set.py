...
def add_property_set_button_pressed(
    ...
    if property_set_tool.is_pset_existing(new_name, som_class):
        ...
        return
    allowed_psets = property_set_tool.get_ifc_names(
        som_class.ifc_mapping[newest_version], newest_version
    )
    parent, mode = property_set_tool.search_for_parent(
        new_name, som_class, allowed_psets
    )
    if mode == 0:
        return
    elif mode in (1, 2):
...
def search_parent(
    ...
    if predefined_psets.name_is_in_predefined_psets(pset_name):
        ...
        if connect_result is None:  # aborted
            return None, 0
        if connect_result:
            parent_pset = predefined_psets.get_pset_by_name(pset_name)
            return parent_pset, 1
    if som_class is not None and property_set_tool.is_name_in_parent_classes(
        pset_name, som_class
    ):
    ...
    if pset_name in allowed_ifc_psets:
