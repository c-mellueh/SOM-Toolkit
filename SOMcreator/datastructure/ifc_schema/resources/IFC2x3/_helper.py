...
def extract_complex_type_names():
    ...
    names = [e.get("name") for e in elements if not e.get("abstract") and e.get("type")]
    return names
...
def get_parent_list(type_name):
    ...
    while not find_parent_classes(parent).startswith("ex"):
...
def get_class_names(t: ElementTree):
    ...
    class_names = [
        cn.text
        for cn in root.findall(".//ApplicableClasses/ClassName")
    ]
