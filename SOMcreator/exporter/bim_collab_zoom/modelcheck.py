...
def _write_smartviewset(
    ...
    etree.SubElement(smartview_set, const.TITLE).text = som_class.name
...
def _write_smartviewsets(
    ...
    for som_class, pset_dict in required_data_dict.items():
        if som_class.is_concept:
            continue
        smartviewsets.append(_write_smartviewset(som_class, pset_dict, author))
...
def build_full_required_data_dict(project: SOMcreator.SOMProject) -> REQUIRED_DATA_DICT:
    required_data = dict()
    for som_class in list(project.get_classes(filter=True)):
        required_data[som_class] = dict()
        for pset in som_class.get_property_sets(filter=True):
            required_data[som_class][pset] = [
