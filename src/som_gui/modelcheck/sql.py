...
def create_tables(db_path):
    ...
    c.execute('''
              ...
              ([GUID_ZWC] CHAR(64) PRIMARY KEY,[GUID] CHAR(64),[Project] TEXT, [ifc_type] TEXT,[x_pos] DOUBLE,
...
def db_create_entity(element: entity_instance, cursor, project, file_name, bauteil_klasse):
    ...
    try:
        cursor.execute(f'''
                  INSERT INTO entities (GUID_ZWC,GUID,Project,ifc_type,x_pos,y_pos,z_pos,datei,bauteilKlassifikation)
                        VALUES
                        ('{guid_zwc}','{guid}','{project}','{ifc_type}',{center[0]},{center[1]},{center[2]},'{file_name}','{bauteil_klasse}')
                  ''')
...
def query_issues(cursor) -> sqlite3.Cursor:
    return cursor.execute(
        "SELECT i.*, e.datei, e.bauteilKlassifikation FROM issues AS i JOIN entities e on i.GUID = e.GUID_ZWC")
